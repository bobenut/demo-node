...
master.begin = function(){
	zkClient = zookeeper.createClient('192.168.222.25:2181', {sessionTimeout:5000});
...
function doResponsiblity(){
	var synces = [syncWorkers(), syncRequesters(), syncTasks()];
...
function syncTasks(){
	return new Promise(function(resolve, reject){
		getTaskNames()
			.then(syncTasksForAll)
			.then(function(results){
				console.log('%s=> sync tasks ok', 
					config.masterName);
				resolve(results);
			})
			.catch(function(error){
				console.error('%s=> sync tasks <%s> error: %s', 
					config.masterName, error.message);
				reject(error);
			})
	});
...
function syncTasksForAll(taskNodeNames){
	return new Promise(function(resolve1, reject1){
...
function syncTask(taskDetail){
	return new Promise(function(resolve, reject){
		...
		.then(function(result){
			console.log('%s=> sync task <%s> ok', 
				config.masterName, result.taskNodeName);
			resolve(result);
		})
		.catch(function(error){
			console.error('%s=> sync task <%s> error: %s', 
				config.masterName, taskDetail.taskNodeName, error.message);
			reject(error);
		});
	});
...
function synAssignedNotDoneTask(taskDetail){
	return new Promise(function(resolve, reject){
		...
		synAssignedNotDoneTask_GetAndWatchAsssignedTaskChildren(taskDetail)
			.then(synAssignedNotDoneTask_updateAssignedDoneTask)
			.then(function(taskDetail){
				console.log('%s=> sync assiged not done task ok', 
					config.masterName, taskDetail.taskNodeName);
				resolve(taskDetail);
			})
			.catch(function(error){
				console.error('%s=> sync assiged not done task error: %s', 
					config.masterName, error.message);
				reject(error);
			});
	});
...
function synAssignedNotDoneTask_GetAndWatchAsssignedTaskChildren(taskDetail){
	return new Promise(function(resolve, reject){
		zkClient.getChildren(
			taskDetail.assignedPath,
			assignedTasksWatcher,
			function(error, children, state){
				if(error){
					console.error('%s=> sync assiged task, getting and watching for task children assigned error: %s',
						config.masterName, error.message);
					reject(error);
					return;
				}
...
function synAssignedNotDoneTask_updateAssignedDoneTask(taskDetail){
	return new Promise(function(resolve, reject){
		if(!(taskDetail.isDispatchedToWorker == true &&
			taskDetail.isWorkerDone == true)){
			console.log('%s=> sync assiged task, task <%s> not yet done, watched', 
				config.masterName, taskDetail.taskNodeName);
			resolve(taskDetail);
			return;
		}
		...
		zkClient.transaction()
			...
			.commit(function(error, results){
				if(error){
					console.error('%s=> sync assiged task <%s>, update assigned done task error: %s', 
						config.masterName, taskDetail.taskNodeName, error.message);
					reject(error);
					return;
				}

				console.log('%s=> sync assiged task, update assigned done task <%s> ok', 
					config.masterName, taskDetail.taskNodeName);
